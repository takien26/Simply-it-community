...
export async function POST(
  ...
  try {
    ...
    const canExtend = currentUser.roleName === 'Admin' || (await hasPermission(currentUser.userId, 'tickets.sla'));
    if (!canExtend) {
      return NextResponse.json({ error: 'Forbidden: Bạn không có quyền gia hạn SLA ticket' }, { status: 403 });
    }
