...
export async function GET(request: NextRequest) {
  try {
    ...
    const canView = currentUser.roleName === 'Admin' || (await hasPermission(currentUser.userId, 'reports.view'));
    if (!canView) {
      return NextResponse.json({ error: 'Forbidden: Bạn không có quyền xem báo cáo kiểm toán' }, { status: 403 });
    }
