...
export async function POST(request: NextRequest) {
  try {
    ...
    const canCreate = currentUser.roleName === 'Admin' || (await hasPermission(currentUser.userId, 'vendors.create'));
    if (!canCreate) {
      return NextResponse.json({ error: 'Forbidden: Bạn không có quyền thêm nhà cung cấp' }, { status: 403 });
    }
