...
export async function PUT(
  ...
  try {
    ...
    if (!canUpdate) {
      return NextResponse.json({ error: 'Forbidden: Bạn không có quyền sửa sơ đồ mặt bằng' }, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (!canDelete) {
      return NextResponse.json({ error: 'Forbidden: Bạn không có quyền xóa sơ đồ mặt bằng' }, { status: 403 });
    }
