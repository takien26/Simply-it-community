...
export async function generateDatabaseBackupData(userEmail: string) {
  ...
  return { backupData, totalRecords };
}

export async function restoreDatabaseFromJson(backupData: any, currentUser: any) {
    if (!backupData || (!backupData.systemSettings && !backupData.users && !backupData.assets)) {
      throw new Error('File sao lưu không đúng định dạng chuẩn ITSM');
    }
