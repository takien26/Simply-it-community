  ...
  const handleConfirmOffboard = async () => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        alert(json.error || t('users.offboard.err_fail', 'Thủ tục thôi việc thất bại.'));
      }
    } catch (err: any) {
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="font-bold text-sm text-white">
                  {t('users.offboard.modal_title', 'Quy Trình Nghỉ Việc & Thu Hồi Tài Sản (1-Click Offboard)')}
                </h3>
                <p className="text-xs text-rose-200">
                  {t('users.offboard.modal_subtitle', 'Thu hồi thiết bị về kho • Giải phóng license • Chuyển giao ticket • Khóa tài khoản')}
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            {loading ? (
              <div className="py-16 text-center text-slate-400 space-y-3">
                ...
                <p className="text-xs font-semibold">{t('users.offboard.loading', 'Đang tra cứu tài nguyên 360° của nhân sự...')}</p>
              </div>
            ) : completedSummary ? (
              ...
              <div className="space-y-4 py-4 animate-in fade-in">
                <div className="p-5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-700 rounded-2xl text-center space-y-2">
                  ...
                  <h4 className="font-black text-base text-emerald-950 dark:text-emerald-200">
                    {t('users.offboard.success_title', 'Đã Hoàn Tất Thủ Tục Nghỉ Việc Thành Công!')}
                  </h4>
                  <p className="text-xs text-emerald-800 dark:text-emerald-300 max-w-md mx-auto">
                    {t('users.offboard.success_desc', 'Tài khoản của nhân sự {name} đã bị vô hiệu hóa. Toàn bộ tài sản được chọn đã hoàn trả về kho sẵn sàng và license đã được giải phóng.')
                      .replace('{name}', completedSummary.user?.fullName || '')}
                  </p>
                </div>

                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">{t('users.offboard.stat_assets', 'Thiết bị thu hồi')}</span>
                    <strong className="text-base font-black text-slate-900 dark:text-white">
                      {completedSummary.revokedAssets?.length || 0}
                    </strong>
                  </div>
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">{t('users.offboard.stat_licenses', 'License giải phóng')}</span>
                    <strong className="text-base font-black text-slate-900 dark:text-white">
                      {completedSummary.revokedLicenses?.length || 0}
                    </strong>
                  </div>
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-800">
                    <span className="text-[10px] font-bold text-slate-500 uppercase block">{t('users.offboard.stat_tickets', 'Ticket xử lý')}</span>
                    <strong className="text-base font-black text-slate-900 dark:text-white">
                ...
                <div className="pt-3 flex items-center justify-center gap-3">
                  ...
                  >
                    ...
                    <span>{t('users.offboard.export_handover_btn', '🖨️ Xuất / In Biên Bản Thu Hồi Đa Tài Sản (A4)')}</span>
                  </button>
                  <button
                    type="button"
                    onClick={onClose}
              ...
              <>
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between flex-wrap gap-2">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {previewData.user.department || t('users.offboard.no_department', 'Chưa có phòng ban')} • {previewData.user.position || t('users.offboard.default_position', 'Nhân viên')}
                    ...
                    <span className="px-2.5 py-1 rounded-full bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-400 border border-rose-300 text-[11px] font-bold">
                      {t('users.offboard.status_badge', 'Sắp chuyển Nghỉ việc')}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                      ...
                      <span>
                        {t('users.offboard.step1_title', '1. Thiết bị phần cứng đang giữ ({count})')
                          .replace('{count}', String(previewData?.assets?.length || 0))}
                      </span>
                    </span>
                    {previewData?.assets?.length > 0 && (
                      ...
                      >
                        {selectedAssetIds.size === previewData.assets.length ? t('users.offboard.deselect_all', 'Bỏ chọn tất cả') : t('users.offboard.select_all', 'Chọn tất cả')}
                      </button>
                    )}
                  </div>
                  ...
                  {(!previewData?.assets || previewData.assets.length === 0) ? (
                    <p className="text-xs text-slate-400 italic bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                      {t('users.offboard.no_assets', '✓ Nhân sự không giữ thiết bị phần cứng nào.')}
                    </p>
                  ) : (
                    <div className="space-y-2">
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-100 text-emerald-800">
                              {t('users.offboard.return_to_inventory', 'Thu hồi về kho')}
                            </span>
                          </label>
                        );
                      })}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                      ...
                      <span>
                        {t('users.offboard.step2_title', '2. Bản quyền phần mềm đang cấp ({count})')
                          .replace('{count}', String(previewData?.licenses?.length || 0))}
                      </span>
                    </span>
                    {previewData?.licenses?.length > 0 && (
                      <button
                        ...
                        className="text-[11px] text-indigo-600 font-bold hover:underline cursor-pointer"
                      >
                        {selectedLicenseIds.size === previewData.licenses.length ? t('users.offboard.deselect_all', 'Bỏ chọn tất cả') : t('users.offboard.select_all', 'Chọn tất cả')}
                      </button>
                    )}
                  </div>
                  ...
                  {(!previewData?.licenses || previewData.licenses.length === 0) ? (
                    <p className="text-xs text-slate-400 italic bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                      {t('users.offboard.no_licenses', '✓ Nhân sự không được gán bản quyền phần mềm nào.')}
                    </p>
                  ) : (
                    <div className="space-y-2">
                ...
                {previewData?.tickets?.length > 0 && (
                  <div className="space-y-2 p-3.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-2xl">
                    <span className="text-xs font-bold text-amber-900 dark:text-amber-200 flex items-center gap-1.5">
                      ...
                      <span>
                        {t('users.offboard.step3_title', '3. Vé hỗ trợ (Ticket) đang mở phụ trách ({count})')
                          .replace('{count}', String(previewData.tickets.length))}
                      </span>
                    </span>

                    <p className="text-[11px] text-amber-800 dark:text-amber-300">
                      {t('users.offboard.tickets_desc', 'Nhân sự đang phụ trách {count} ticket chưa hoàn tất. Vui lòng chọn kỹ thuật viên nhận bàn giao:')
                        .replace('{count}', String(previewData.tickets.length))}
                    </p>
                      ...
                      >
                        {previewData.itStaff?.map((st: any) => (
                          <option key={st.id} value={st.id}>
                            {t('users.offboard.transfer_ticket_to', 'Chuyển giao cho: {name} ({dept})')
                              .replace('{name}', st.fullName)
                              .replace('{dept}', st.department || 'IT')}
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300">
                    {t('users.offboard.notes_label', 'Ghi chú biên bản bàn giao & thu hồi:')}
                  </label>
                  <textarea
                    rows={2}
                    value={notes}
                    ...
                    placeholder={t('users.offboard.notes_placeholder', 'Ghi chú lý do thôi việc, bàn giao linh kiện...')}
                  />
                </div>
              </>
            )}
          ...
          {!completedSummary && (
            <div className="flex items-center justify-between px-6 py-3.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 shrink-0">
              <span className="text-[11px] text-slate-400">
                {t('users.offboard.footer_tip', 'Nhân sự sẽ bị khóa đăng nhập ngay sau khi xác nhận.')}
              </span>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span>{submitting ? t('users.offboard.confirming', 'Đang Xử Lý Thu Hồi...') : t('users.offboard.confirm_btn', 'Xác Nhận Nghỉ Việc & Thu Hồi')}</span>
                </button>
              </div>
            </div>
          )}
