...
export function ManageableDropdown({
  ...
  const effectivePlaceholder = placeholder ?? (isEn ? '-- Select item --' : '-- Chọn mục --');
  const effectiveEmptyLabel = emptyLabel ?? (isEn ? '-- Unlinked / None --' : '-- Không chọn / Bỏ liên kết --');
  const effectiveSearchPlaceholder = searchPlaceholder ?? (isEn ? 'Quick search...' : 'Tìm kiếm nhanh...');
      ...
      {label && (
        <div className="flex items-center justify-between mb-1">
          ...
          {onQuickAddClick ? (
            ...
            >
              <Plus className="w-3 h-3" /> <span>{isEn ? 'Add New' : '+ Thêm mới'}</span>
            </button>
          ) : onAdd ? (
            <button
              type="button"
            ...
            >
              <PlusCircle className="w-3 h-3" /> {isEn ? 'Quick add' : 'Thêm nhanh'}
            </button>
          ) : null}
        </div>
      )}
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0 flex-1 pr-2">
          {selectedItem ? (
            ...
            <span className="text-slate-400 font-normal truncate text-xs">{effectivePlaceholder}</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          {allowEmpty && selectedItem && (
            <span
              ...
              title={isEn ? 'Clear selection' : 'Bỏ chọn'}
          ...
          {isAdding && onAdd && (
            <div className="p-2.5 bg-indigo-50/80 border-b border-indigo-100 shrink-0 space-y-2">
              <p className="text-[11px] font-bold text-indigo-900 flex items-center gap-1">
                <Plus className="w-3 h-3 text-indigo-600" /> {isEn ? 'Add new item:' : 'Thêm mục mới:'}
              </p>
              <div className="flex items-center gap-1.5">
                <input
                  ...
                  placeholder={isEn ? 'Enter name...' : 'Nhập tên...'}
          ...
          <div className="overflow-y-auto flex-1 p-1 divide-y divide-slate-50">
            ...
            {searchTerm.trim() && !items.some((i) => i.name.toLowerCase() === searchTerm.trim().toLowerCase()) && onAdd && (
              ...
              >
                <span className="flex items-center gap-1.5 truncate">
                  ...
                  <span className="truncate">{isEn ? `Create & Select: "${searchTerm.trim()}"` : `Tạo mới & Chọn: "${searchTerm.trim()}"`}</span>
                </span>
            ...
            {filteredItems.length === 0 && !searchTerm.trim() ? (
              <div className="p-4 text-center text-xs text-slate-400">
                {isEn ? 'No data available' : 'Chưa có dữ liệu nào'}
              </div>
            ) : filteredItems.length === 0 && searchTerm.trim() && !onAdd ? (
              <div className="p-4 text-center text-xs text-slate-400">
                {isEn ? 'No matching results' : 'Không tìm thấy kết quả phù hợp'}
              </div>
            ) : (
                    ...
                    <div className="flex items-center space-x-1 shrink-0">
                      ...
                      {onEdit && (
                        <button
                          ...
                          title={isEn ? 'Edit this item' : 'Sửa tên mục này'}
                      ...
                      {onDelete && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (confirm(isEn ? `Are you sure you want to delete "${item.name}"?` : `Bạn có chắc muốn xóa "${item.name}"?`)) {
                              onDelete(item.id, item.name);
                            }
                          }}
                          title={isEn ? 'Delete this item' : 'Xóa mục này'}
